...
export const ChainOfThoughtHeader = memo(({ className, children, ...props }: ChainOfThoughtHeaderProps) => {
  ...
  return (
    <Collapsible onOpenChange={setIsOpen} open={isOpen}>
      <CollapsibleTrigger
        className={cn(
          "flex w-full cursor-pointer items-center gap-2 rounded-md px-1 py-1 text-muted-foreground text-sm transition-colors hover:text-foreground",
          className
        )}
        {...props}
      >
        <BrainIcon className="pointer-events-none size-4 shrink-0" />
        <span className="pointer-events-none flex-1 text-left">{children ?? "Chain of Thought"}</span>
        <ChevronDownIcon
          className={cn("pointer-events-none size-4 shrink-0 transition-transform", isOpen ? "rotate-180" : "rotate-0")}
        />
      </CollapsibleTrigger>
    </Collapsible>
  )
...
export const ChainOfThoughtStep = memo(
  ({
    ...
    <div
      className={cn("fade-in-0 slide-in-from-top-2 flex animate-in gap-2 text-sm", stepStatusStyles[status], className)}
      {...props}
    >
      <div className="relative mt-0.5">
        <Icon className="size-4" />
        <div className="absolute top-7 bottom-0 left-1/2 -mx-px w-px bg-border" data-chain-line="" />
      </div>
